...
public class BoomBoxBeilsteinAppActivity extends Activity {
    ...
    OnClickListener skipListener = new OnClickListener() 
	{
		...
		public void onClick(View v) 
		{
			try 
			{
				if(conn != null)
				{
					Toast toast = Toast.makeText(getApplicationContext(), "Spule vor bis Live, kleinen Moment bitte...", Toast.LENGTH_LONG);
					toast.show();
					remoteService.skipForward(getStreamURL());

					Animation exitAnimation = AnimationUtils.loadAnimation(getApplicationContext(), R.anim.moveanimation);
    ...
    OnClickListener stopListener = new OnClickListener() 
	{
		...
		public void onClick(View v) 
		{
			try 
			{
				...
				{
					playButton.setBackgroundResource(R.drawable.playbutton);
					Toast toast = Toast.makeText(getApplicationContext(), "Stoppe Livestream...", Toast.LENGTH_LONG);
					toast.show();
					remoteService.stop();
				}
			} 
    ...
    /** Gets the recent URL from the server where it is stored under "www.boomboxbeilstein.de/StreamUri.txt". */
    ...
    //TODO: ev. Möglichkeit zum aufnehmen des live streams
}